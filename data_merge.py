...
class DataEnricher:
    ...
    def apply_header_formatting(self, original_excel_path: str, output_excel_path: str, 
        ...
        try:
            ...
            # Use read_only=False to access styles, but keep it lightweight
    ...
    def enrich_data(self, excel_path: str, table_name: str, 
        ...
        for batch_start in range(0, len(df_excel), BATCH_SIZE):
            ...
            # Batched path using tuple IN over reference columns to avoid row reordering
            # 1) Build composite keys per row and track rows with missing refs
            row_keys = {}
            ...
            if unique_keys:
                ...
                # 3) Build lookup dict from DB rows keyed by tuple(reference_columns)
                lookup = {}
                for r in results:
            ...
            # 4) Emit rows in original order, mapping by key; fill missing as None
            for idx, row in batch_df.iterrows():
        ...
        logger.info(f"  Total rows: {len(df_enriched)}")
            ...
            try:
                ...
                if output_extension == '.csv':
                    ...
                    try:
                        ...
                        logger.warning(f"Could not apply header formatting (file still saved): {format_error}")
            except Exception as e:
